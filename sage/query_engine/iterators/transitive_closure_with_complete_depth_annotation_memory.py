...
class TransitiveClosureIterator(PreemptableIterator):
    ...
    def __init__(self, subject: str, obj: str, iterators: List[PreemptableIterator], var_prefix: str, bindings: List[Dict[str, str]] = None, current_depth: int = 0, min_depth: int = 1, max_depth: int = 10, complete: bool = True):
        ...
        if bindings is not None and bindings[0] is not None:
            ...
            for i in range(0, current_depth):
                ...
                if self._bindings[i]['?complete']:
                    self._visited[first_node][node] = 1
                else:
    ...
    def backtrack(self):
        ...
        """
        while self._current_depth > 0 and not self._iterators[self._current_depth].has_next():
            self.update_last_visited()
            self._bindings[self._current_depth] = None
            self._current_depth -= 1

    def update_last_visited(self):
        current_binding = self._bindings[self._current_depth]
        if current_binding is not None and current_binding['?complete']:
            first_node = self.first_node()
            last_node = self.last_node()
            self._visited[first_node][last_node] = 1
    ...
    def expand_current_path(self) -> int:
        ...
        if i < self._max_depth - 1:
            ...
            return i + 1
        elif i == self._max_depth - 1:
            self._iterators[i + 1].next_stage(last_binding)
            if self._iterators[i + 1].has_next():
