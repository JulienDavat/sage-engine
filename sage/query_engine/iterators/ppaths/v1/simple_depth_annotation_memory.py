...
class TransitiveClosureIterator(DLSIterator):
    ...
    def __init__(self, source: str, path: str, obj: str, forward: bool, iterators: List[PreemptableIterator], mu: Optional[Dict[str, str]] = None, bindings: List[Dict[str, str]] = None, current_depth: int = 0, min_depth: int = 1, max_depth: int = 10, id: Optional[int] = None):
    ...
    async def next(self) -> Optional[Dict[str, str]]:
        if self.has_next():
            ...
            if self._iterators[depth].has_next():
                ...
                node = self.get_node(depth)
                if not self.must_explore(node, depth) or self.is_goal_reached():
                    self._bindings[depth] = None
                    return None, False, 0
                self.update_depth(node, depth)
