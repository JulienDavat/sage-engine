...
def parse_query(query: str, dataset: Dataset, default_graph: str, control_tuples: ControlTuplesBuffer) -> Tuple[PreemptableIterator, dict]:
    ...
    Args:
      ...
      * control_tuples: The list of control tuples generated by this query during this quantum.
    ...
    try:
        ...
        cardinalities = list()
        iterator = parse_query_alt(logical_plan, dataset, [default_graph], cardinalities, control_tuples, as_of=start_timestamp)
...
def parse_query_alt(node: dict, dataset: Dataset, current_graphs: List[str], cardinalities: dict, control_tuples: ControlTuplesBuffer, query_vars: Optional[Set[str]] = None, as_of: Optional[datetime] = None) -> PreemptableIterator:
    """Recursively parse node in the query logical plan to build a preemptable physical query execution plan.
    ...
    if node.name == 'SelectQuery':
        ...
        if node.datasetClause is not None:
            graphs = [format_term(graph_iri.default) for graph_iri in node.datasetClause]
        return parse_query_alt(node.p, dataset, graphs, cardinalities, control_tuples, query_vars=query_vars, as_of=as_of)
    elif node.name == 'ConstructQuery':
        graphs = current_graphs
        if node.datasetClause is not None:
            graphs = [format_term(graph_iri.default) for graph_iri in node.datasetClause]
        child=parse_query_alt(node.p, dataset, graphs, cardinalities, control_tuples, query_vars=query_vars, as_of=as_of)
        return ConstructIterator(child,convert_construct_template(node.template))
    elif node.name == 'Reduced':
        child = parse_query_alt(node.p, dataset, current_graphs, cardinalities, control_tuples, query_vars=query_vars, as_of=as_of)
        return ReducedIterator(child)
    elif node.name == 'Project':
        projected_vars = list(map(lambda t: '?' + str(t), node.PV))
        variables = set()
        child = parse_query_alt(node.p, dataset, current_graphs, cardinalities, control_tuples, query_vars=variables, as_of=as_of)
        # return child
        ...
        triples = list(localize_triples(node.triples, current_graphs))
        iterator, bgp_cardinalities, bgp_variables = build_left_join_tree(triples, query_vars, dataset, current_graphs[0], control_tuples, as_of=as_of)
        # cardinalities += bgp_cardinalities
        for variable in list(bgp_variables):
            query_vars.add(variable)
        return iterator
    elif node.name == 'Union':
        left = parse_query_alt(node.p1, dataset, current_graphs, cardinalities, control_tuples, query_vars=query_vars, as_of=as_of)
        right = parse_query_alt(node.p2, dataset, current_graphs, cardinalities, control_tuples, query_vars=query_vars, as_of=as_of)
        return BagUnionIterator(left, right)
    elif node.name == 'Filter':
        expression = parse_filter_expr(node.expr)
        iterator = parse_query_alt(node.p, dataset, current_graphs, cardinalities, control_tuples, query_vars=query_vars, as_of=as_of)
        return FilterIterator(iterator, expression)
    elif node.name == 'Extend':
        bgp_iterator=parse_query_alt(node.p, dataset, current_graphs, cardinalities, control_tuples, query_vars=query_vars, as_of=as_of)
...
def parse_update(query: dict, dataset: Dataset, default_graph: str, as_of: Optional[datetime] = None) -> Tuple[PreemptableIterator, dict, ControlTuplesBuffer]:
